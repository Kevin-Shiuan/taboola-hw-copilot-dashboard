...
type MessageUpdate =
  | {
      type: MessageUpdateType.AppendTurn
      userId: string
      assistantId: string
      text: string
    }
...
function messagesReducer(messages: Message[], update: MessageUpdate): Message[] {
  switch (update.type) {
    ...
    case MessageUpdateType.AppendToken:
      return messages.map((m) => (m.id === update.assistantId ? { ...m, content: m.content + update.token } : m))
    ...
    case MessageUpdateType.RestartStreaming:
      return messages.map((m) => (m.id === update.assistantId ? { ...m, content: '', streaming: true } : m))
  }
}
